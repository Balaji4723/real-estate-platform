...
function Field({ label, children, span = false }) {
  return (
    <label className={`block ${span ? "sm:col-span-2" : ""}`}>
      <span className="block text-xs font-mono uppercase tracking-wide text-ink/60 dark:text-paper/60 mb-1.5">
        {label}
      </span>
      {children}
    </label>
...
const inputClass =
  "w-full bg-transparent border border-ink/20 dark:border-chalk/30 px-3 py-2.5 text-sm font-body focus:border-brass transition-colors";
...
export default function AddPropertyForm({ onCreated }) {
  ...
  function handleFiles(e) {
    ...
    const combined = [...files, ...picked].slice(0, 8);
    setFiles(combined);
  }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <Field label="Title" span>
          <input
            ...
            placeholder="e.g. 2BHK Apartment near Marina Beach"
