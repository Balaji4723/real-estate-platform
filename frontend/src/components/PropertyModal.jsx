...
export default function PropertyModal({ property, onClose, onDeleted, token, onLoginNeeded }) {
  ...
  async function handleDelete() {
    if (!token) {
      addToast("Please log in as admin to remove listings.", "info");
      onLoginNeeded?.();
      return;
    }
        ...
        >
          ...
          {images.length > 1 && (
            <div className="flex gap-2 p-3 overflow-x-auto bg-ink/5 dark:bg-paper/5">
              {images.map((img, i) => (
                <motion.button key={img} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}
                  onClick={() => setActiveImage(i)}
                  className={`shrink-0 w-16 h-16 overflow-hidden border-2 transition-colors ${i === activeImage ? "border-brass" : "border-transparent"}`}>
          ...
          <div className="p-5 sm:p-7">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
              ...
              <div>
                <h2 className="font-display text-2xl font-semibold text-ink dark:text-paper">{property.title}</h2>
              ...
              <span className={`shrink-0 text-[10px] font-mono uppercase tracking-wide px-2 py-1 border ${property.status === "Available" ? "border-moss text-moss" : "border-clay text-clay"}`}>
                {property.status}
              </span>
            </motion.div>
            ...
            <div className="mt-6 flex items-center justify-between text-xs font-mono text-ink/40 dark:text-paper/40 border-t border-ink/10 dark:border-chalk/15 pt-4">
              ...
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                ...
                <Trash2 size={14} />
                {deleting ? "Removing…" : token ? "Remove listing" : "Admin only"}
              </motion.button>
            </div>
          </div>
        </motion.div>
